...
export const Tasks = createSlice({
  ...
  reducers: {
    ...
    removeTask: (state, action) => {
      state.value = state.value.filter((task) => task._id !== action.payload.id);
    },
    ...
    markDueDate: (state, action) => {
      state.dueDate.forEach((e) => {
        if (e._id === action.payload.id) {
        state.dueDate[state.dueDate.indexOf(e)].status = action.payload.status; 
        }
      });
    },
    ...
    deleteFromPending: (state,action) => {
      state.pendingTasks = state.pendingTasks.filter((task) => task._id !== action.payload.id);
    },
    deleteFromCompleted: (state,action) => {
      state.completedTasks = state.completedTasks.filter((task) => task._id !== action.payload.id);
    },
    deleteFromdueDate : (state,action) => {
      state.dueDate = state.dueDate.filter((task) => task._id !== action.payload.id);
    },
    setAll: (state,action) => {
...
export const {
  ...
  markCompletePending,
  markpendingComplete,
  getDueDate,deleteFromdueDate,markDueDate,setAll
} = Tasks.actions;
